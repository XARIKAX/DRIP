...
export function DashboardPreview() {
  ...
  return (
    <div className="panel">
      {/* Window chrome */}
      <div className="flex items-center justify-between border-b border-panel-line px-5 py-3.5">
        <div className="flex items-center gap-1.5" aria-hidden>
          <span className="block h-2 w-2 rounded-full bg-panel-3" />
          <span className="block h-2 w-2 rounded-full bg-panel-3" />
          <span className="block h-2 w-2 rounded-full bg-cyan/70" />
        </div>
        <span className="font-mono text-nano uppercase tracking-widest text-panel-faint">
          osinko · dashboard
        </span>
      ...
      <div className="p-5 md:p-6">
        ...
        {/* Streams */}
        <div className="mt-5 border border-panel-line">
          ...
          {open.map((s) => {
            ...
            return (
              <div key={s.id} className="border-b border-panel-line px-4 py-3.5 last:border-b-0">
                <div className="flex items-center justify-between gap-4">
                  <div className="flex min-w-0 items-center gap-3">
                    <TokenMark symbol={s.symbol} size={28} dark />
                    <div className="min-w-0">
                      <div className="text-[13px] font-bold tracking-tight text-panel-text">
                        {s.symbol}
                      </div>
                      <div className="font-mono text-nano uppercase text-panel-faint">
                        {MODE_LABEL[s.mode]}
                      </div>
                    </div>
                  </div>
                  <div className="shrink-0 text-right">
                    ...
                    <div className="num mt-1.5 text-nano uppercase text-panel-faint">
                      of ${fmt(s.totalUsd)}
                    </div>
                  </div>
                </div>
        ...
        {/* Holdings */}
        <div className="mt-5 hidden border border-panel-line md:block">
          <div className="border-b border-panel-line px-4 py-3">
          ...
          {top.map((h) => (
            <div
              key={h.symbol}
              className="grid grid-cols-[1fr_auto_auto_auto] items-center gap-4 border-b border-panel-line px-4 py-3 last:border-b-0"
            >
              <div className="flex min-w-0 items-center gap-3">
                <TokenMark symbol={h.symbol} size={24} dark />
                <span className="text-[13px] font-bold tracking-tight text-panel-text">{h.symbol}</span>
              </div>
              <span className="num text-[12px] text-panel-faint">{fmt(h.amount, 4)}</span>
              <span className="num text-[12px] text-panel-text">${fmt(h.valueUsd)}</span>
